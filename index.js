...
const foodSchema = new mongoose.Schema({
    ...
    hasDrink:{type: Boolean, required: true, default: false},
...
const MenuItem = mongoose.model("MenuItem", foodSchema, "Menu")
...
app.post("/menu/test", async (req, res) => {
    const freshItem = await new MenuItem({
...
app.get("/menu/value", async (req, res) =>{
    const items = await MenuItem.find({cost:{$lte:7.50}})
})
